#Name: Naman Garg
#Roll No: B032
#Aim: Implementation of a divide & conquer algorithm - Merge Sort


# function to sort the 2 subarrays and merge them
def merge(m, n, r):
    ...
    while(i < len(copy1) and j < len(copy2)):
        ...
        if(copy1[i] <= copy2[j]):
            ...
            l[k] = copy2[j]
            j, k = j + 1, k + 1
    # When 1 subarray gets exhausted of elements
    else:
        #remaining elements will be filled in the sorted array as it is
        while(i < len(copy1)):  
            l[k] = copy1[i]
            i, k = i+1, k+1
        while(j < len(copy2)):
...
# Repeatedly divides the array into subarrays to sends to merge
def mergesort(m, r):
    ...
    if(m < r):
        ...
        print(f'{copy1} and {copy2} on sorting and merging ->{l[m:r+1]} -> comparisons ={c}')
